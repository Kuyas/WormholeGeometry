...
(* ::Text:: *)
(*The generally covariant Levi-Civita tensor, in d space(time) dimensions, is Subscript[*)
(*\!\(\*OverscriptBox[\(\[Epsilon]\), \(~\)]\), Subscript[\[Mu], 1]... Subscript[\[Mu], d]]=(|det Subscript[g, \[Alpha]\[Beta]]|)^(1/2) Subscript[\[Epsilon], Subscript[\[Mu], 1]... Subscript[\[Mu], d]], where Subscript[\[Epsilon], Subscript[\[Mu], 1]... Subscript[\[Mu], d]] is the Levi-Civita symbol, which in turn returns the sign of the permutation that brings {Subscript[\[Mu], 1,...,] Subscript[\[Mu], d]} into {0,1,2,3,..., d-1} (or into {Subscript[s, i]+1,Subscript[s, i]+2,Subscript[s, i]+3,...,Subscript[s, i]+d}, for some integer Subscript[s, i]) and is zero otherwise. In flat spacetime, this means we adhere to the covention that Subscript[*)
(*\!\(\*OverscriptBox[\(\[Epsilon]\), \(~\)]\), 0123...d-1] =1 regardless of the signature of the metric. Note: Mathematica has a built-in LeviCivitaTensor that appears to implement what we would call the Levi-Civita tensor in d-dimensional flat Euclidean space in Cartesian coordinates.*)
...
(*		\[FivePointedStar]Subscript[A, Subscript[\[Mu], 1]... Subscript[\[Mu], d-n]]\[Congruent](1/n!)Subscript[*)
(*\!\(\*OverscriptBox[\(\[Epsilon]\), \(~\)]\), Subscript[\[Mu], 1]... Subscript[\[Mu], d-n] Subscript[\[Nu], 1]... Subscript[\[Nu], n]]A^(Subscript[\[Nu], 1]... Subscript[\[Nu], n]).*)
...
(* ::Subsection::Closed:: *)
(*GeodesicSystem, GeodesicLagrangians*)
...
(* ::Text:: *)
(*ToTensor returns a Tensor. This may need to be updated whenever we create new arguments for Tensor.*)
...
(*	 Subscript[F, \[Mu]\[Nu]] = *)
(*\!\(\*SubscriptBox[\(\[PartialD]\), \(\([\)\(\[Mu]\)\)]\)\[CapitalPhi]^1Subscript[\[PartialD], \[Nu]]](\[CapitalPhi]^2)\[Congruent] \!\( *)
(*\*SubscriptBox[\(\[PartialD]\), \(\[Mu]\)]*)
...
(*	n^\[Mu]=*)
(*\!\(\*OverscriptBox[\(\[Epsilon]\), \(~\)]\)^(\[Mu] \[Alpha]\[Beta]\[Gamma])\!\( *)
...
(*Because of the anti-symmetric properties of the covariant Levi-Civita tensor *)
(*\!\(\*OverscriptBox[\(\[Epsilon]\), \(~\)]\)^(\[Mu] \[Alpha]\[Beta]\[Gamma]), we can see directly that n^\[Mu] \!\( *)
(*\*SubscriptBox[\(\[PartialD]\), \(\[Mu]\)]*)
